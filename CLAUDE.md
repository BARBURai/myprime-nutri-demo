...
- **Bump `VERSION` by 0.01 on every change**, and **state the new version number in the chat reply** (the owner tracks versions; it also shows in the UI). Current version: `3.10`.
...
## v3.14 - Search: prefix matching in the national DB + two-stage search (no AI burn on partial typing)
Pilot report (Ron, on v3.13): typing "ש"/"שנ"/"שני" showed "לא נמצאו תוצאות" (sometimes labeled USDA) and only the full word "שניצל" returned results. ROOT CAUSE: data.gov.il's datastore full-text search matches WHOLE WORDS only, so every partial prefix returned empty; the client then ran the full fallback chain (OFF -> AI translate -> USDA) on every partial keystroke - confusing UX AND a paid AI call per keystroke. The CSV substring fallback in api/il-food.js appears to no longer return anything in production (data.gov.il CSV download presumed broken; unverifiable from the sandbox). Fixes:
- **SERVER (api/il-food.js): prefix search.** `datastoreSearch` now first tries a PostgreSQL tsquery PREFIX query (`q:*`, plain=false) so "שנ" matches שניצל/שניצלונים from the 2nd letter, falling back to the plain whole-word search. Prefix variant only attempted for a single clean token (tsquery syntax breaks on spaces). Also scrubbed 2 legacy em dashes from comments. NOTE: data.gov.il is unreachable from the sandbox - Ron must verify after deploy via /api/il-food?q=שנ.
- **CLIENT: two-stage search.** Stage A (350ms): catalog + Israeli DB (fast, cheap). Stage B (1200ms, only if il found nothing AND query >= 3 chars): OFF -> AI-translate -> USDA. Partial prefixes while typing no longer reach the expensive chain at all. A `searchRunRef` run-id guards every setState so an older slower response can never overwrite a newer query (fixes the out-of-order race from the code review).
- **CLIENT: translation cache.** `translateFoodToEnglish` caches successes in TRANSLATE_CACHE for the session - the same word never costs a second AI call.
- **CLIENT: honest search states.** While the chain is still running with nothing found yet, a centered "מחפשת…" row (spinner) shows; "לא נמצאו תוצאות" appears only after the FULL chain ended. The DB-source label renders only when there are actual DB results (no more misleading "USDA" header over an empty list).
- VERSION 3.13->3.14. esbuild clean (transform + bundle-with-externals); api/il-food.js syntax-checked with node. qa/ harness not in the supplied zip, so check-logic.mjs not run here. 0 em/en dashes in all three files. CHANGED FILES: src/App.jsx, api/il-food.js, CLAUDE.md.


Pilot report (Ron, after v3.12): while typing in food search the input got hidden by the keyboard/results; single-letter queries returned irrelevant substring matches; and "שניצל" sometimes found results, sometimes "לא נמצא". None were caused by v3.12 (that only touched the shared catalog) - these are pre-existing search behaviors. All three fixed client-side:
- **Sticky input.** The meal chips + search box are now wrapped in a `position: sticky; top: 0` container (background C.panel) inside the sheet's scroll area, so the keyboard and incoming results scroll BENEATH them and the input stays in view while typing.
- **2-char minimum.** The search effect and the local-pool `filtered` now require `query.trim().length >= 2`, so a single character no longer triggers a search or substring-matches every food containing that letter (was the "ש" -> מבושל/שועל/שרי noise + flicker). The DB-source label and the "no results" message are likewise gated to >=2 chars; the existing search hint now stays visible until 2 chars are typed.
- **Stable Israeli-DB results.** data.gov.il's datastore is intermittently empty/slow, so the same query flip-flopped between found/not-found. `searchIsraeliDB` now retries once on an empty result and caches only SUCCESSES for the session (IL_CACHE) - once a food is found it stays found, while a transient miss can still recover on the next keystroke (misses are never cached).
- VERSION 3.12->3.13. esbuild clean (transform + bundle-with-externals). qa/ harness not in the supplied zip, so check-logic.mjs not run here; validated via esbuild + review. 0 em/en dashes. CHANGED FILES: src/App.jsx, CLAUDE.md.


Pilot heads-up (Ron): searching surfaced a long combined entry under "מהקטלוג שלנו" ("חצי ג'בטה + חציל צלוי + ביצה קשה + ...") that one woman had logged as a single product via the AI "כמוצר אחד" combine option. It is one person's plate, irrelevant to everyone else and a small privacy leak. Direction (Ron): keep SIMPLE single products shared (e.g. "נס קפה") but drop the combined personal meals.
- `catalogAdd`: now also returns early when `item.combo` is set OR the name contains " + " (the established combine marker, see the isCombo check), so combined meals are never written to the shared catalog. Single foods (no " + ") are unaffected and still shared.
- `catalogSearch`: results are now also filtered to exclude any name containing " + ", so meals already stored before this guard disappear from "מהקטלוג שלנו" for all users without needing a server purge.
- Not changed: the AI "לפי הרכיבים" path still logs each component as its own single item (those are simple foods, fine to share); only the "one product" combined name is excluded.
- VERSION 3.11->3.12. esbuild clean (transform + bundle-with-externals). qa/ harness not in the supplied zip, so check-logic.mjs not run here; validated via esbuild + review. 0 em/en dashes. CHANGED FILES: src/App.jsx, CLAUDE.md.


Three reports from the pilot (Eti x2, Liron x1), all fixed client-side in src/App.jsx.
- **BUG A - search showed 0 kcal for everything (Eti, "שניצל").** The `/api/il-food` endpoint returns each item with macros NESTED under `per100` ({ name, per100:{ kcal,... } }), but `searchIsraeliDB` read the FLAT `it.kcal` (undefined -> 0). Surfaced now only because data.gov.il's datastore went live and il finally returns rows (before, il was empty and we fell through to OFF/USDA). FIX: read `it.per100 || {flat}` in `searchIsraeliDB` AND `searchUSDA` (defensive for either shape). Server files were correct - this was purely the client misreading the shape.
- **BUG A side-effect - catalog poisoning.** While il returned 0, any 0-kcal item a user logged was written to the shared catalog at 0 and kept showing under "מהקטלוג שלנו". FIX: `catalogAdd` now returns early unless per100.kcal>0 (no new poison); `catalogSearch` filters out any per100.kcal<=0 rows (hides legacy ones). The AI-reconcile path already skipped 0 via its `c.per100.kcal` truthiness check. (Physical purge of old 0-rows in Upstash deferred - needs api/catalog.js; not user-visible anymore.)
- **BUG B - AI calories jumped to thousands (Eti, "חציל אפוי בתנור עם ספריי שמן" -> 1370).** `localFoodMatch` used a SUBSTRING test (`nq.includes(fn)`), so the staple `שמן` (884 kcal/100g) matched inside the dish name and `reconcileWithDb` scaled the whole 155g dish at oil density (884*1.55=1370). The AI's own estimate (~40) was correct; only the reconciled chip was wrong. FIX: `localFoodMatch` now matches the staple only as CONSECUTIVE WHOLE WORDS and only when the query is essentially that staple (<= staple word-count + 1), so "שמן" no longer matches inside "ספריי שמן" / a multi-word dish. Same risk existed for חמאה/סוכר/דבש/קמח/לחם - all covered.
- **BUG C - yesterday's breakfast vanished (Liron).** Append/persist logic was correct (never overwrites a day). Root cause: auto-backup ran ONCE A DAY and early (`if (last===today) return`, +4s), so anything logged later that day was never in the cloud; after storage eviction (iOS PWA ~7-day) or a device switch, restore brought back the early snapshot missing those entries. FIX: backup is now DEBOUNCED after every change (12s) and also flushes on `visibilitychange`->hidden and `pagehide`, uploading only when the content changed since the last upload (bkSentRef). Closes the loss window. NOTE: backup is still opt-in; women who never enabled it have no safety net - product decision pending (Ron) whether to encourage/auto-enable.
- Not in this batch (Iki's request): an explicit "how much of the dish did you eat?" portion option for composite dishes. Today the AI "ספרי לי מה אכלת" path already handles partial portions by description; a dedicated UI control is a separate small feature awaiting Ron's decision.
- VERSION 3.10->3.11. esbuild clean (transform + bundle-with-externals). NOTE: the qa/ harness was NOT included in the supplied zip (only src/App.jsx + CLAUDE.md), so check-logic.mjs was not run here - validated via esbuild parse/bundle + targeted review instead. 0 em/en dashes. CHANGED FILES: src/App.jsx, CLAUDE.md.


Approved fix for the Brazil report (and any user not near UTC). Two parts:
- ROOT BUG: date strings ("YYYY-MM-DD") were parsed via new Date(str) = UTC midnight, then read with device-local .getDay()/.getDate(). West of UTC (e.g. Brazil UTC-3) that lands on the PREVIOUS local day, so addDays/sundayOf/dowOf/prettyDate/the daystrip/week+day numbers all shifted a day. FIX: new helpers parseDay() (Date.UTC parse) + fmtDay()/pad2(); addDays, sundayOf, dowOf, prettyDate, listSundays, the daystrip (dd), the weight/cal/steps chart labels, both CheckinCard dateLines, the collection Shabbat skip, and the last-weight label all now use parseDay + getUTC* -> pure calendar math, identical in every timezone (verified under TZ=America/Sao_Paulo vs Asia/Jerusalem).
- POLICY (Ron): the app works by the USER'S OWN local timezone, not Israel. ymd(new Date()) stays the DEVICE's local date (her "today"); check-in reveal hour stays her local hour. So a user abroad sees her own correct day automatically.
- ymd kept device-local (NOT forced to Israel tz). No UI change.
- This is item #1 of 3. #2 (onboarding timezone prompt) and #3 (localize the 19:00 reminder) are pending exact-copy / approach approval before coding.
- VERSION 3.09->3.10. esbuild clean, check-logic 7/7, 0 dashes. CHANGED FILES: src/App.jsx, CLAUDE.md.

## v3.09 - Subtitle line under the "הערות לאפליקציה" heading
In the NotesFab feedback bubble, added one line between the heading and the textarea (exact wording supplied by Ron): "כל הערה תעזור לנו לשפר 💜 ואם תרצי שנחזור אלייך, אנא הוסיפי בהודעה את מספר הטלפון שלך". Title row marginBottom 12->8.
- VERSION 3.08->3.09. esbuild clean, check-logic 7/7, 0 dashes. CHANGED FILES: src/App.jsx, CLAUDE.md.
...
- aiNutritionChat max_tokens 800 -> 1200 (the main cause: long replies/many items truncated mid-JSON).
- On parse failure: salvage just the "reply" string via regex if present, else a clean retry message ("רגע, לא הצלחתי לנתח את זה במלואו..."). Never display raw JSON remnants.
- raw stored in history is now the valid JSON when parsed, or a CLEAN JSON string ({reply,done:false,items:[]}) on fallback - so a failed turn no longer poisons the conversation.
- VERSION 3.05->3.06. esbuild clean, check-logic 7/7, 0 dashes. CHANGED FILES: src/App.jsx, CLAUDE.md.

## v3.05 - Combined products reused by SERVING (מנה), not grams
From pilot (Ron screenshot): a "כמוצר אחד" combo like "נס קפה + חלב + סוכר" was saved unit g and showed irrelevant gram/spoon/portion measures on re-add. Fix: combos are now reused by serving.
- The AI combined commit passes servingG = total grams of the combo.
- commit() favorite reconstruction: when p.servingG, the favorite gets measures [{מנה, servingG}, {100 ג׳,100}], def 0, and combo:true (instead of generic 100/כף/כפית).
- Qty screen: for a combo (food.combo OR name includes " + ") it skips the measuresForUnit merge and shows only its own measures; pickFood opens a combo on the מנה unit. So "1 מנה" = the whole combo and +/- adds 2, 3 servings (2 coffees).
- Re-adding a combo (both the quick + and the name->qty path) now re-passes servingG so the serving favorite is preserved, not overwritten with generic measures.
- NOTE: applies to combos saved from v3.05 on; a combo saved earlier needs to be re-logged once (re-run the AI combine) to gain the מנה measure.
- VERSION 3.04->3.05. esbuild clean, check-logic 7/7, 0 dashes. CHANGED FILES: src/App.jsx, CLAUDE.md.

## v3.04 - Favorites multi-add UX: top Done button, clearer hints, cups for drinks
From pilot testing (Ron) on the אחרונים ומועדפים multi-add:
- "סיום" button was only at the BOTTOM (lost under a long list). Now there is an explanation line at the TOP (after the meal chips) and a "סיום · N נוספו" button right under it (when N>0), AND one kept at the bottom - reachable from anywhere.
- After "✓ נוסף" it was unclear you can tap the NAME to change quantity. Now an added row's subtitle becomes "לכמות אחרת - הקישי על השם" (brand color) right at that item. Subtitle also now shows מ"ל for ml items (was hardcoded ג׳).
- Qty screen lacked CUPS for drinks (only גרם/100/כף/כפית), irrelevant for coffee. Now the unit pills MERGE measuresForUnit(food.unit) with the food's own measures (dedup by label, g>1). For liquids the standard set leads, so כוס/פחית/חצי ליטר appear; for solids the food's specific measure stays first then מנה sizes follow.
- VERSION 3.03->3.04. esbuild clean, check-logic 7/7, 0 dashes. CHANGED FILES: src/App.jsx, CLAUDE.md.

## v3.03 - Staples in local spine, smarter reconcile, custom quantity in favorites multi-add
From pilot testing (Ron):
- BASIC STAPLES added to FOODS: סוכר (387/0/0/100, כפית 4g/כף 12g), שמן (884 fat), חמאה (717), דבש (304), קמח (364), מלח (0), קפה שחור (2, ml). So when the AI decomposes a meal (coffee+milk+sugar), these resolve INSTANTLY from memory instead of a network lookup. (milk/egg/coffee-with-milk were already there.)
- lookupProduct rewritten with order: (1) session cache RECON_CACHE, (2) local FOODS spine, (3) our shared catalog, (4) Israeli DB, (5) USDA, (6) OFF. Was: straight to the 3 external DBs. So staples = no network, and anything resolved once is cached. localFoodMatch (exact name or query-includes-foodname), reconKey = name|en.
- SKIP RE-LOOKUP on quantity-only change: RECON_CACHE (module Map, session) stores the per100 result by name|en; reconcileWithDb rescales by grams after, so 2->3 teaspoons reuses the cached value with zero network. Fixes Ron's "why does it re-check sugar at 2 and again at 3 tsp" + "sugar is basic, should be in memory".
- FAVORITES MULTI-ADD now supports CUSTOM QUANTITY: tapping the item NAME (not +) opens the qty screen, and when origin is history the commit uses keepOpen, marks the row added (addedKeys), and RETURNS to the favorites list instead of closing - so you can add several items each at its own quantity (1 cup coffee, 2 tsp sugar...). The + stays the quick default-quantity add. Hint text updated. (Editing an already-added item's quantity is still via the diary.)
- VERSION 3.02->3.03. esbuild clean, check-logic 7/7, 0 dashes. CHANGED FILES: src/App.jsx, CLAUDE.md.
...
AI CACHE (api/ai.js): for first-turn TEXT queries (one user msg, no image, no prior assistant) it normalizes the text -> key `aicache:<normQuery>` and, on a hit, returns the stored Anthropic response WITHOUT calling Claude (header x-ai-cache:hit) and increments usage:<day>:cachehits - skips quota too (checked before the rate limiter). On miss + success it caches the raw response (POST form for large values) 30d, only if it produced items. Photos/multi-turn never cached.
SHARED CATALOG (api/catalog.js NEW): cat:<normName> -> {name, per100, unit, source, seen, ts}. POST (x-user-id required) validates via plausiblePer100 + rejects source:"manual" (manual stays private), upserts (seen++, higher sourceRank overwrites values), 180d sliding TTL. GET ?q= searches (filters key names by normQuery, GETs <=8) and returns client food objects (measures 100/כף/כפית, fromCatalog). Admin ?secret=NOTIFY_SECRET&action=list (sorted by seen) / &action=del&key=... for review/prune by Ron/Anat.
CLIENT (src/App.jsx): catalogAdd() fires on every commit for non-manual items (best-effort, derives per100, never blocks); catalogSearch() runs in the search debounce -> new "מהקטלוג שלנו" section between the local pool and the external DBs (deduped by name vs local), tap adds/picks like any result; no-results condition now also checks catResults.
REPORT (api/usage-report.js): added cacheHits row + estimated saved NIS (cacheHits x avg cost/call) so the morning email shows the cache savings.
- Quality model: Atwater+range gate on entry, DB/barcode (verified) outranks AI (estimated) on merge, seen-count = consensus signal, source badge stays visible, admin list/del for the high-use long tail. This operationalizes Layer-1 (food-data accuracy) QA continuously.
- KEYS cat:* per search is fine at pilot scale; switch to SCAN/index before large scale (noted).
- VERSION 2.13->3.0 (milestone: shared catalog + AI cache infrastructure). esbuild clean, check-logic 7/7, 0 dashes. CHANGED FILES: src/App.jsx, api/ai.js, api/catalog.js, api/usage-report.js, lib/foodcheck.js, CLAUDE.md.

## v2.13 - Pilot feedback: combine AI components / flexible quantity / multi-add + daily usage report email
APP (src/App.jsx):
- **Combine AI result as one product**: when an AI analysis returns 2+ components (e.g. coffee+milk+sugar), the result card now offers a choice "לפי הרכיבים" (current) vs "כמוצר אחד" - the latter logs ONE entry (summed kcal/p/f/c, editable name defaulting to the joined component names) and saves ONE favorite, so it re-adds in one tap. State aiAsOne/aiOneName; commit branches in the aiDoneItems footer (IIFE). NOT called a "recipe" (that word is reserved for the future real recipe booklet).
- **Flexible quantity (qty step rewrite)**: replaced the "מידת בית pills + ±10g stepper" with a unit-x-count model. Unit pills = base גרם/מ"ל + the food's household measures; tapping a measure sets it (count 1). The −/+ stepper steps by ONE unit (so כפית x2 = 10g, not a 5->15 jump), and the count is an editable numeric input (type exact grams, or 2 teaspoons, etc.). Shows "= X גרם" when a measure is active. State qUnit (null=base), reset in pickFood. nut/commit unchanged (still driven by grams).
- **Multi-add from אחרונים ומועדפים**: the + no longer closes the dialog. App commit got an optional 2nd arg keepOpen (`if(!keepOpen) setModal(null)`). History + calls commit(item, true), marks the row "✓ נוסף" (green) and counts; a "סיום · N נוספו ליומן" button closes. State addedKeys.
USAGE REPORT (api/ai.js + api/usage-report.js + vercel.json):
- api/ai.js: after a SUCCESSFUL call, logs data.usage tokens to daily Redis counters usage:<day>:calls/in/out/photos (Israel day, ~8d TTL, best-effort, never blocks).
- NEW api/usage-report.js: cron, gated to ~07:00-08:00 Jerusalem (single 05:00 UTC cron, DST-safe), idempotent per day (SET NX usage:rep:<day>). Reads yesterday's counters + the per-user ai:day:*:<day> distribution, estimates cost (tokens x Sonnet price x USD_NIS), emails a Hebrew summary via Resend to Ron@myprime.co.il. Auth = CRON_SECRET / NOTIFY_SECRET (same as notify). Manual test: ?secret=...&force=1[&day=YYYY-MM-DD].
- NEEDS (Ron, in Vercel): RESEND_API_KEY (from resend.com). Optional: REPORT_FROM (verified sender), REPORT_TO, AI_PRICE_IN/OUT, USD_NIS. Until RESEND_API_KEY is set the endpoint returns a JSON preview (no email).
- VERSION 2.12->2.13. esbuild clean, check-logic 7/7, 0 dashes. CHANGED FILES: src/App.jsx, api/ai.js, api/usage-report.js, vercel.json, CLAUDE.md.

## v2.12 - Manual food entry (type values off the label, no AI)
- New: when a product isn't in search/barcode, the user can type its nutrition values manually - no AI, no limit. Added a "הזנה ידנית" method (new step "manual" in AddModal) reachable from: the method screen (new Pencil tile), the barcode-not-found screen, and the search no-results state.
- Form fields: meal chips, product name, amount + unit (g/ml), and per-100 values from the label (kcal, protein, fat, carbs). saveManual computes the entry for the amount and calls the existing commit().
- Reuse: commit already auto-saves every logged food to `favorites` with per100, so manual products persist. Also added favorites into the search pool (localPool = favorites + FOODS, deduped by name), so a manually-added product now appears when searched by name next time (matches Ron's expectation that it's saved for reuse). All on-device (localStorage), like the rest.
- VERSION 2.11->2.12. esbuild clean, check-logic 7/7, 0 em/en dashes. CHANGED FILES: src/App.jsx, CLAUDE.md.

## v2.11 - Keep hard cap at 30; warn on the last allowed call (revert the 60 ceiling)
- Ron didn't ask for the HARD_DAILY=60 ceiling added in 2.10 - he wants to stay at 30 for now. Removed HARD_DAILY. The cap is a hard 30 again (call 31+ blocked). But instead of a bare rejection, the LAST allowed call (dayCount === DAILY_LIMIT, i.e. #30) is still answered AND sets `x-ai-limit: soft`, so the client appends "זו הייתה המנה האחרונה במכסת ה-AI להיום..." after the answer - she's forewarned on her last analysis rather than getting a no-answer block. (Tradeoff: a 31st typed message is still blocked; that's inherent to a strict 30 cap. The only way to never block a typed message is to let it through, which is what the 60 ceiling did.)
- App VERSION 2.10->2.11. esbuild clean, check-logic 7/7, 0 em/en dashes. CHANGED FILES: api/ai.js, src/App.jsx, CLAUDE.md.

## v2.10 - Soft AI daily limit (answer first, then notify - don't waste typed input)
- Problem (Ron): a user types a long detailed food message and gets the "limit reached" rejection instead of an answer - her effort is wasted, very frustrating. Fixed: the daily AI limit is now SOFT. api/ai.js: added HARD_DAILY (env AI_HARD_LIMIT, default DAILY_LIMIT*2 = 60). Between DAILY_LIMIT (30) and HARD_DAILY the request is STILL processed and answered, and the server sets header `x-ai-limit: soft`. Only above HARD_DAILY does it truly block (cost guard). 
- Client aiNutritionChat reads `x-ai-limit`; when soft, appends a gentle note AFTER the answer ("דרך אגב, הגעת למכסת ה-AI היומית... מכאן אפשר דרך חיפוש או ברקוד"). So she always gets her analysis; the limit only warns. Cost note: soft-limit calls do hit the API (between 30-60).
- App VERSION 2.09->2.10. esbuild clean, check-logic 7/7, 0 em/en dashes. CHANGED FILES: api/ai.js, src/App.jsx, CLAUDE.md.

## v2.09 - Raise AI daily limit 10 -> 30
- Pilot users were hitting the 10/day AI-analysis cap (even when batching foods into one detailed message), getting "הגעת למכסת ניתוחי ה-AI להיום". api/ai.js DAILY_LIMIT default 10 -> 30. Still overridable per-deploy via the AI_DAILY_LIMIT env var (no code change) for fine-tuning vs API cost. Burst limit + PHOTO_LIMIT (70/program) unchanged.
- App VERSION 2.08->2.09 (api change shipped as a release). esbuild clean, check-logic 7/7, 0 em/en dashes. CHANGED FILES: api/ai.js, src/App.jsx, CLAUDE.md.

## v2.08 - Fix NaN calorie total (one bad entry broke the whole ring)
- BUG (seen in pilot, "NaN קלוריות" on the day ring): a food entry with a missing/invalid kcal (e.g. a USDA/search item with no calorie value, like the Magnum the user picked) made the unguarded sum `dayLog.reduce((s,e)=>s+e.kcal,0)` evaluate to NaN, breaking the entire day's calorie display.
- Fixed at two levels: (1) nutritionFor is now NaN-proof - coerces grams + each per100 field via `Number(x)||0`, never returns NaN; (2) guarded the kcal sums that weren't: consumed (line ~984), activity actKcal (~986), and the recommendation engine (~4251) now use `(e.kcal||0)`/`(a.kcal||0)`. A bad item now contributes 0 instead of breaking the total.
- VERSION 2.07->2.08. esbuild clean, check-logic 7/7, 0 em/en dashes. CHANGED FILES: src/App.jsx, CLAUDE.md.
