...
// תמונת הפתיחה של מסך הקורס העצמאי, מהקובץ ששלח רון. במסך הזה היא מחליפה גם
// את הלוגו וגם את משפט הפתיחה, כי היא אומרת לאן היא נכנסה בלי מילה אחת.
export const GLOW_HERO = "/glow-hero.jpg";
// הפלטה של הסעיף. בגרסה הקודמת היא נגזרה מהגרדיאנט שבלוגו עצמו, ורון פסל את
// התוצאה: "ורדרדים מידי ילדותי מידי וזול". לכן המג׳נטה נשארת בלוגו בלבד,
// והסעיף סביבו לובש חצילית עמוקה ואפור רך. ככה הלוגו הוא הדבר הצבעוני היחיד
// על המסך, ולא אחד מתוך כמה שמתחרים זה בזה.
export const GLOW_C = {
  ink: "#3E2148",     // חצילית עמוקה, לכותרות לאייקונים ולחצים
  accent: "#3E2148",  // עיגול החץ, באותו טון
  bg: "#F5F1F7",      // אפור רך עם נטייה קלה לסגול, ולא ורוד
  line: "#E2DCE6",
};
...
export const GLOW_FULL_TITLE = "מיי פריים Glow";
...
/* ============================================================
   מפתח הסימון של שיעור גלו
   ============================================================
   רון, 14 בספטמבר 2026: "למה אתה לא שם את הווי לפי הסרטון ולא לפי המקום
   ברשימה, זה נראה לי בכל מקרה יותר הגיוני."

   הוא צדק. בכל 60 ימי התוכנית המקום ברשימה הוא הדבר היחיד שקיים, כי יש שם גם
   דפי משימה ותמונות בלי שום סרטון, והרשימה של כל יום קבועה ואף פעם לא נדחף
   בה שיעור באמצע. **בגלו זה אחרת: יש שתי רשימות שמתחלפות זו בזו**, ארבעת
   שיעורי הדמו ו-28 שיעורי הקורס, ולכן המקום ברשימה חסר משמעות. אישה שראתה
   בדמו את שיעור 3, שהוא מקום 1 שם, הייתה מקבלת ווי על שיעור 1ב׳ שהוא מקום 1
   בקורס ומעולם לא נפתח לה.

   מכאן הווי, הלב ומונה הצפיות של גלו נשמרים לפי מזהה הסרטון עצמו, ולכן
   החלפת רשימה או הוספת שיעור באמצע הקורס אינן מזיזות שום סימון. */

// האם הקורס המלא פתוח לאישה הזאת. `hasGlowFull()` אומר רק שהרשימה אינה ריקה
// בקוד, וזו שאלה אחרת לגמרי. הדגל נכתב ב-src/App.jsx בכל תשובה של השער.
export function glowFullAccess() {
  try { return localStorage.getItem("myprime_glow_full") === "1"; } catch (e) { return false; }
}

// הרשימה שהאישה הזאת רואה בפועל.
export function glowListFor() {
  return glowFullAccess() && hasGlowFull() ? GLOW_FULL_DAY : GLOW_DAY;
}

// מזהה הסרטון במקום i ברשימה שלה, או ברשימה שנמסרה במפורש.
export function glowVideoAt(i, list) {
  const ls = (list || glowListFor()).lessons;
