...
console.log("\nבנק התשובות");
{
  ...
  // ניסוח שמבטיח "תיקנו" אינו יושב בבנק כל עוד אין תיקון מאושר. נשבר ב-v5.90 והוחזר
  // ב-v5.92: ההבטחה כבר הייתה יוצאת למשתתפת אמיתית.
  // "תיקנו לך את תאריך ההתחלה" נשאר: את זה הפקידה עושה בעצמה במסך לפני שהיא שולחת.
  // מה שאסור הוא הבטחה על תיקון בקוד שלא אושר ולא נדחף.
  check("אין בבנק הבטחה על תיקון בקוד שלא אושר",
    !SEED.some(function (e) { return /(המשקל|היעד|הכמות|השדה)[\s\S]{0,80}(תיקנו|אנחנו מתקנים)/.test(e.a); }));
  check("ולבאג אין הצעה", onSeed("לא מצליחה להזין משקל 44").length === 0);
  check("ולהערה בת שתי מילים אין", onSeed("המשקל שגוי").length === 0);

  // הערה בלי ניסוח מוכן חייבת לומר למה. שתיקה במקום הזה נקראת כאילו המסך לא עובד.
  const talkSrc = html.match(/var TALK = \[[\s\S]*?\n\];[\s\S]*?function talkWhy\(text\)\{[\s\S]*?\n\}/);
  check("talkWhy קיימת ב-admin.html", !!talkSrc);
  const talkWhy = new Function(talkSrc[0] + "; return talkWhy;")();
  [
    ["רף המשקל הוא החלטה ולא תקלה", "[נכתב 19.08] לא מצליחה להזין משקל 44"],
    ["היעד בדוח מול הפעילות", "אין התחשבות בשינוי היעד היומי בהתאם לפעילות הגופנית שביצעתי"],
    ["שדה הכמות", "כשניסיתי לשנות את כמות הגר בפרוסת הלחם זה לא התאפס לי כלומר המספר 1 נשאר"],
    ["הערה קצרה מדי", "המשקל שגוי"],
    ["משהו שנמחק לה", "הייתה לי רשימה של מזונות שהכנסתי וזה מחק לי אותה"],
...
console.log("\nחיפוש, תגי תוכנית ותאריך במסך הניהול");
{
  ...
  // שלושה מקומות מ-v6.78: הכרטיס, השורה ברשימה, והבלוק במסך "בעיות בגיליון".
  check("ותג הביטול קיים בשלושת המקומות", (html.match(/grp dngpill">ביטלה/g) || []).length === 3);
  ...
  // 2ב. כרטיס פתוח של מישהי אחרת נשאר על המסך בזמן חיפוש חדש. נסגר עכשיו לבד,
  //     ורק בחיפוש: הנפילה אחורה חייבת להישאר כדי שאפשר יהיה לפתוח אישה מתוך
  //     תור המענה גם כשהיא במחזור אחר מזה שמסונן.
  check("כרטיס פתוח נסגר כשהיא אינה בתוצאות החיפוש",
    /if\(searching && SEL && !list\.some\(function\(w\)\{ return w\.email === SEL; \}\)\)\{/.test(html));
  check("והנפילה אחורה נשמרה, אחרת תור המענה נשבר",
    /\|\| DATA\.women\.filter\(function\(w\)\{ return w\.email === SEL; \}\)\[0\];/.test(html));
  check("גם 'חסרות קבוצה' מאפס את הכרטיס הפתוח",
    /MISSING=!MISSING; EDITING=""; VIEW=""; SEL="";/.test(html));
  ...
  //     עונה תשובה אחת והאפליקציה מציגה אחרת.
  {
    ...
    // הברקוד: השאלות בשני המקומות מנוסחות אחרת בכוונה, כי בבנק הן הלשון שבה
    // המשתתפת באמת כתבה וזה מה שהדירוג נתפס עליו. **התשובה חייבת להיות אחת.**
    // רון, 28 באוגוסט 2026: "תשאיר רק אותה על הברקוד."
    {
      const bc = pick(app, "סרקתי ברקוד והערכים לא תואמים לאריזה. מה עושים?");
      check("התשובה על הברקוד קיימת בשאלות ותשובות שבאפליקציה", bc.length > 0);
      const bank12 = pick(html, "סרקתי ברקוד והערכים לא תואמים לאריזה, הקלוריות שגויות");
      const bank13 = pick(html, "יש בעייה בחישוב חלבון של מוצר, יש בו יותר חלבון ממה שמופיע");
      check("ושתי הרשומות בבנק נושאות בדיוק את אותה תשובה", bc.length > 0 && bank12 === bc && bank13 === bc,
        bank12.slice(0, 40) + " || " + bank13.slice(0, 40));
      // הניסוח הישן פתח ב"את צודקת", וזה מענה לתלונה ולא ערך ברשימה.
      check("והניסוח הישן ירד משני המקומות", !/את צודקת, והערכים שהופיעו היו/.test(html));
    }
    // הפעילות הגופנית: השאלה בבנק היא הלשון שבה המשתתפת כתבה, ובאפליקציה היא
    // מנוסחת כשאלה. **התשובה אחת.**
    {
      const a = pick(app, "אפשר להזין פעילות גופנית בכל יום?");
      const b = pick(html, "הייתי שמחה אם אפשר היה לציין פעילות ספורטיבית בכל הימים ולא רק בימים שבתוכנית");
      check("התשובה על הפעילות הגופנית קיימת בשני המקומות", a.length > 0 && b.length > 0);
      check("והיא זהה מילה במילה", a.length > 0 && a === b, a.slice(0, 40) + " || " + b.slice(0, 40));
    }
    // סך הקלוריות בכל ארוחה. בבנק זו הלשון שבה המשתתפת כתבה, ובאפליקציה שאלה.
    {
      const a = pick(app, "איך רואים כמה קלוריות אכלתי בכל ארוחה?");
      const b = pick(html, "באפליקציה של קלוריה רואים כל ארוחה כמה קלוריות אכלתי");
      check("התשובה על הקלוריות בארוחה קיימת בשני המקומות", a.length > 0 && b.length > 0);
      check("והיא זהה מילה במילה", a.length > 0 && a === b, a.slice(0, 40) + " || " + b.slice(0, 40));
    }
    // חצאי שעות השינה יושבים בבנק בלבד: השדה גלוי במסך ואין מה ללמד עליו.
    {
      const b = pick(html, "בדיווח שעות שינה כדאי להוסיף אפשרות לדווח חצאים");
      check("התשובה על חצאי שעות השינה קיימת בבנק", b.length > 0);
      check("והיא נוקבת בדוגמה שהיא יכולה להקליד", /6\.5/.test(b), b.slice(0, 40));
    }
