...
console.log("\nהדגשה, והמקום שנפתח לטקסט\n");
// רון: "תדגיש את הטקסט שאומר לשינוי עוצמת הקול, להגדיל את הסרטון למסך מלא,
// מסך הסרטונים". הביטוי יושב בשדה נפרד ולא בתוך המחרוזת, כמו בשאלות ותשובות.
for (const phrase of ["לשינוי עוצמת הקול", "להגדיל את הסרטון למסך מלא", "מסך הסרטונים"])
  check(`מודגש בשתי ההדרכות: ${phrase}`, (app.match(new RegExp(`b: "${phrase}"`, "g")) || []).length === 2);
check("והרינדור מדגיש אותו", (app.match(/withBold\(s\.text, s\.b\)/g) || []).length === 2);
// **החלונית נמתחה למטה.** רון ראה שורה אחת יורדת מתחת לקצה המסך, ונמדד בדפדפן
// בגובה 640 ובגובה 580: לפני השינוי שלוש שקופיות נחתכו, ואחריו כולן נכנסות.
check("החלונית תופסת כמעט את כל הגובה", (app.match(/maxHeight: "98%"/g) || []).length === 2);
check("והשוליים סביבה צומצמו", (app.match(/justifyContent: "center", padding: 8, zIndex: 48/g) || []).length === 2);
