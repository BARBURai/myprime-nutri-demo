...
console.log("\nהדלת האחורית: עריכת המשקל בפרופיל, בלי שהקצב משתנה\n");
...
if (back) {
  const b = backWorst;
  console.log(`  ${back} צירופים שבהם אישה מתחת לרף מקבלת יעד קלורי נמוך מהתחזוקה שלה,`);
  console.log(`  אם היא עורכת את המשקל בפרופיל והקצב נשאר כפי שהיה.`);
