...
// הכלל על כמות היה בנוי על רשימת מאכלים ("ביצה, תפוח, בננה וכו'"), ולכן הוא נשבר על
// כל מאכל שלא היה ברשימה. משתתפת כתבה "פריכית" והמנוע שאל אותה כמה. הכלל דקדוקי
// עכשיו, וזו הבדיקה שמונעת חזרה לרשימה בשקט.
{
  const app = readFileSync(new URL("../src/App.jsx", import.meta.url), "utf8");
  const must = [
    ["הכלל דקדוקי ולא לפי רשימה", "הכלל על כמות הוא דקדוקי ולא לפי רשימת מאכלים"],
    ["יחיד בלי מספר פירושו אחד", "שם מאכל שנכתב בלשון יחיד ובלי מספר פירושו אחד, תמיד"],
    ["חל גם על מאכל שלא מוכר", "גם אם לא שמעת על המאכל הזה מעולם"],
    ["חל גם כשיש כמה מאכלים במשפט", "גם כשמנויים כמה מאכלים באותו משפט, עם פסיקים או בלי"],
    ["אסור לבקש אישור על אחד", "אל תבקשי אישור על ההנחה הזאת"],
    ["מותר לשאול על רבים בלי מספר", "המאכל נכתב בלשון רבים בלי מספר"],
    ["מותר לשאול על מאכל בתפזורת", "מאכל בתפזורת שאין לו יחידה טבעית"],
  ];
  for (const [name, needle] of must) {
    if (app.indexOf(needle) !== -1) console.log(`עובר  | ${name}`);
    else { console.log(`נכשל  | ${name}\n        לא נמצא בהנחיות: ${needle}`); failed++; }
  }
}

console.log(`\n${CASES.length + 7 - failed} מתוך ${CASES.length + 7} עברו.`);
if (failed) {
