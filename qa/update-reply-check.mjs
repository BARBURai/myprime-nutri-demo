...
// נמדד בדפדפן ברוחב של טלפון: הסרגל נשבר לשלוש שורות ומגיע ל-89 פיקסלים, ובעוד
// שהוא היה `absolute` הוא כיסה 89 פיקסלים מראש האפליקציה, כלומר את סרגל התאריכים
// כמעט במלואו. רון: "אני עדיין לא רואה כמעט בכלל את הסרגל של התאריכים למעלה."
// עכשיו הוא בזרימה, תופס את הגובה שלו, והאפליקציה מתחילה מתחתיו בכל מספר שורות.
check("סרגל הבדיקות יושב בזרימה ולא מרחף על האפליקציה",
  /id="mp-devbar" style=\{\{ flexShrink: 0,/.test(app) &&
  !/id="mp-devbar" style=\{\{ position: "absolute"/.test(app));
// וההזחה של החריץ יושבת בתוך הסרגל עצמו, ולכן הפס שמתחתיו אינו מוסיף אותה שוב.
check("והפס אינו מחבר את הזחת החריץ פעמיים",
  /top: DEV \? `\$\{devTop\}px` :/.test(bar) &&
  !/calc\(env\(safe-area-inset-top, 0px\) \+ \$\{devTop\}px\)/.test(bar));
