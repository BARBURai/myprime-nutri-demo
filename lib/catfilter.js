// המסנן שקובע מה מוצג בחיפוש המזון לכל הנשים. v7.48, ומ-v7.50 על ערכים מתווית בלבד.
//
...
// **מוצרי חלב חייבים יצרן או אחוז שומן. v7.50, החלטת רון:** "מה זה קוטג' גנרי? זה יכול
// להיות קוטג' 5%, 3%, 1%... אם בשם המוצר אין את היצרן או את אחוזי השומן, אין מה להראות
// אותם. וגנרי יש לנו במקומות אחרים." חל על שם שמתחיל באחת המילים ברשימה בלבד, ולכן
// "פשטידת גבינה" אינה מושפעת. יצרן שאינו ברשימה, בלי אחוז, מוסתר, וזה הכיוון הבטוח.
const DAIRY_FIRST = new Set("קוטג יוגורט גבינה גבינת חלב שמנת לבן אשל לבנה קפיר ריקוטה מעדן".split(" "));
export const DAIRY_BRANDS = "תנובה שטראוס טרה יטבתה דנונה מולר יופלה יופלי גד פרו go אלפרו צבר תלמה שופרסל עמק פיראוס גולן".split(" ");
const HAS_PCT = /\d+(\.\d+)?\s?%/;
export const isDairy = (raw) => DAIRY_FIRST.has(searchKey(cleanName(raw)).split(" ")[0]);
export function dairyProblem(raw) {
  if (!isDairy(raw)) return null;
  if (HAS_PCT.test(raw)) return null;
  if (searchKey(cleanName(raw)).split(" ").some((w) => DAIRY_BRANDS.includes(w))) return null;
  return "generic";
}

// **השם שנשמר מהזנה ידנית ל-100 גרם. v7.51, רעיון של רון:** במוצר חלב, גרם שומן ל-100
// גרם הוא בדיוק אחוז השומן, והמספר כבר בשדה שהיא מילאה. לכן "קוטג'" עם 5 גרם שומן נשמר
// "קוטג' 5%", ויצרן שהקלידה נוסף אחריו. אחוז שכבר כתוב בשם, או יצרן שכבר כתוב בו,
// אינם נוספים פעמיים. **שדה שומן ריק אינו אפס**, ולכן אינו מוסיף אחוז.
// האפליקציה משתמשת בזה גם ליומן שלה וגם לחיפוש המשותף, כדי ששניהם יראו אותו שם.
export function labelName(raw, brand, fat) {
  let n = String(raw || "").trim();
  const b = String(brand || "").trim();
  if (n && fat !== "" && fat != null && isDairy(n) && !HAS_PCT.test(n)) {
    const v = Math.round(Number(fat) * 10) / 10;
    if (Number.isFinite(v) && v >= 0 && v <= 100) n += ` ${v}%`;
  }
  if (n && b && !searchKey(n).includes(searchKey(b))) n += ` ${b}`;
  return n;
}
